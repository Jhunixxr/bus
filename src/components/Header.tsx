...
export function Header() {
  ...
  return (
    <header className="bg-white dark:bg-gray-900 text-gray-900 dark:text-white shadow-lg relative z-50 transition-colors duration-300">
      <div className="container mx-auto px-4">
        <div className="flex items-center justify-between h-20">
          ...
          <nav className="hidden lg:flex items-center space-x-8">
            <Link 
              to="/" 
              className="hover:text-amarillo-dorado dark:hover:text-yellow-400 transition-colors flex items-center space-x-1 group"
            >
            ...
            <Link 
              to="/search" 
              className="hover:text-amarillo-dorado dark:hover:text-yellow-400 transition-colors group"
            >
            ...
            <Link 
              to="/promotions" 
              className="hover:text-amarillo-dorado dark:hover:text-yellow-400 transition-colors relative group"
            >
              <span className="group-hover:scale-105 transition-transform inline-block">
                {t('header.promotions')}
              </span>
                  ...
                  {languages.map((lang) => (
                    <button
                      ...
                      className={`w-full px-4 py-2 text-left hover:bg-gray-100 dark:hover:bg-gray-700 transition-colors flex items-center space-x-2 ${
                        language === lang.code ? 'bg-amarillo-dorado/10 text-amarillo-dorado' : ''
                      }`}
                    >
                      <span>{lang.flag}</span>
                      <span>{lang.name}</span>
        ...
        {isMenuOpen && (
          <div className="lg:hidden absolute top-20 left-0 right-0 bg-white dark:bg-gray-900 border-t dark:border-gray-700 shadow-xl animate-slide-up">
            <nav className="flex flex-col p-4 space-y-3">
              <Link
                to="/"
                className="py-3 hover:text-amarillo-dorado dark:hover:text-yellow-400 transition-colors flex items-center space-x-2"
              ...
              <Link
                to="/search"
                className="py-3 hover:text-amarillo-dorado dark:hover:text-yellow-400 transition-colors"
                onClick={() => setIsMenuOpen(false)}
              >
                {t('header.search')}
              </Link>
              <Link
                to="/promotions"
                className="py-3 hover:text-amarillo-dorado dark:hover:text-yellow-400 transition-colors flex items-center"
                onClick={() => setIsMenuOpen(false)}
              >
                {t('header.promotions')}
